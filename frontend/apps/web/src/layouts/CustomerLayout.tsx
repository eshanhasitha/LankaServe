...
export default function CustomerLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] text-slate-900">
      ...
      <main className={`ml-[260px] ${isMessagesPage ? 'h-screen bg-white' : 'min-h-screen bg-[#F3F4F6]'}`}>
        {!isMessagesPage ? (
          <header className="sticky top-0 z-[1200] flex h-[70px] items-center justify-between border-b border-slate-200 bg-white px-8">
            {isProviderProfileHeader ? (
              <div className="flex items-center gap-4">
                ...
                <h1 className="text-xl font-bold">{providerProfileTitle}</h1>
              </div>
            ) : isJobDetailsHeader ? (
              <div className="flex items-center gap-2 text-sm text-slate-500">
                <NavLink className="hover:text-[#2F4DA0]" to="/customer/my-jobs">My Jobs</NavLink>
                <span className="material-symbols-outlined text-sm">chevron_right</span>
                <span className="font-medium text-slate-900">{jobDetailsTitle}</span>
              </div>
            ) : headerTitle ? (
              <div className="flex items-center gap-4">
