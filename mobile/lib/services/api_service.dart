...
class ApiService {
  ...
  Future<Map<String, dynamic>> get(
    ...
    final response = await _send(
      endpoint,
      (baseUrl) => _client.get(_uri(baseUrl, endpoint, query), headers: _headers()),
    );
  ...
  Future<http.Response> _send(
    ...
    for (final baseUrl in _prioritizedBaseUrls()) {
      ...
      try {
        final response = await request(baseUrl).timeout(Duration(seconds: AppConstants.apiTimeoutSeconds));
  ...
  Map<String, dynamic> _decode(http.Response response) {
    ...
    if (payload is! Map<String, dynamic>) {
      throw ApiException('Unexpected API response format', statusCode: response.statusCode);
    }
